"""
Discord Agent-Importer – Option C (Agentenmodus)
================================================
Liest einen YAML-Blueprint (Datei- oder HTTP-URL), auditiert den Server
und synchronisiert Rollen, Kategorien, Channels und Berechtigungen idempotent.

Features
- Slash-Commands: /dryrun, /sync, /audit, /lockdown, /unlock
- Readonly- & Staff-only-Overwrites exakt nach Blueprint
- Webhook-/Bot-Post-Rechte (z. B. #ad-wins)
- Agent-Loop (optional): pollt YAML-Quelle in Intervallen, diffed & synced
- Safeties: keine Löschungen; rate-limit-aware; Logs in Konsole

Setup
1) Python 3.11+
2) pip install -U discord.py pyyaml requests python-dotenv
3) .env im gleichen Ordner:
   DISCORD_TOKEN=xxxxx
   GUILD_ID=123456789012345678
   AGENT_SOURCE_URL=https://.../blueprint.yaml   # optional (aktiviert Agent)
   AGENT_POLL_SEC=900                            # optional (Sekunden)
4) Start: python agent_importer.py
"""
...
# --- tiny HTTP server for Render Web Service (keepalive) ---
# Wenn du NICHT auf Render als Web Service deployst, kannst du diesen Block drin lassen – stört nicht.
...
AGENT_SOURCE_URL = os.getenv("AGENT_SOURCE_URL")  # HTTP(S) URL oder file:// Pfad oder lokaler Pfad
...
def load_blueprint(source: str) -> Dict[str, Any]:
    """Load YAML from http(s) or local path."""
    ...
    if not isinstance(data, dict):
        raise ValueError("Blueprint YAML muss ein Mapping sein.")
    return data
...
async def ensure_text_channel(
    ...
    staff_only = bool(flags.get("staff_only"))
    allow_bot_post = bool(flags.get("allow_bot_post"))

    if readonly:
        overwrites[guild.default_role] = discord.PermissionOverwrite(view_channel=True, send_messages=False)
        if team:
            overwrites[team] = discord.PermissionOverwrite(view_channel=True, send_messages=True, manage_messages=True)
        if allow_bot_post and bot_role:
            overwrites[bot_role] = discord.PermissionOverwrite(view_channel=True, send_messages=True)
    ...
    if ch:
        await ch.edit(category=category, topic=topic or ch.topic, overwrites=overwrites)
        return ch
...
async def ensure_voice_channel(guild: discord.Guild, category: discord.CategoryChannel, name: str) -> discord.VoiceChannel:
    ch = discord.utils.get(guild.voice_channels, name=name)
    if ch:
        await ch.edit(category=category)
        return ch
    log.info(f"Create voice channel: 🔊 {name}")
    return await guild.create_voice_channel(name=name, category=category)

async def ensure_stage_channel(guild: discord.Guild, category: discord.CategoryChannel, name: str):
    ...
    try:
        return await guild.create_stage_channel(name=name, category=category)
    except AttributeError:
        # Fallback, falls Stage nicht verfügbar ist
        return await guild.create_voice_channel(name=name, category=category)

async def apply_blueprint(guild: discord.Guild, blueprint: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure roles/categories/channels exist and match overwrites. No destructive deletes."""
    ...
    for rname in roles:
        r = await ensure_role(guild, rname)
        role_map[rname] = r
        await asyncio.sleep(0.2)
        if rname not in [x.name for x in guild.roles]:
    ...
    for cat in blueprint.get("categories", []) or []:
        ...
        if category.name not in [c.name for c in guild.categories]:
            summary["categories_created"] += 1
        await asyncio.sleep(0.2)

        for item in cat.get("channels", []) or []:
            ch_name = item.get("name")
            ch_type = (item.get("type") or "text").lower()
            topic = item.get("topic")
            flags = item.get("flags", {}) or {}

            if ch_type == "text":
                before = discord.utils.get(guild.text_channels, name=ch_name)
                await ensure_text_channel(guild, category, ch_name, topic, flags, team, bot_role)
                ...
                await ensure_voice_channel(guild, category, ch_name)
                await asyncio.sleep(0.3)
                if before:
                    summary["channels_updated"] += 1
                else:
                ...
                await ensure_stage_channel(guild, category, ch_name)
                await asyncio.sleep(0.3)
                if before:
                    summary["channels_updated"] += 1
                else:
...
async def dryrun_blueprint(guild: discord.Guild, blueprint: Dict[str, Any]) -> str:
    ...
    for cat in blueprint.get("categories", []) or []:
        ...
        for item in cat.get("channels", []) or []:
            ...
            if not ch:
                lines.append(f"+ create {ch_type} channel: {cname}/#{ch_name}")
            else:
                lines.append(f"~ update {ch_type} channel: {cname}/#{ch_name}")
    return "\n".join(lines)
...
async def on_ready():
    ...
    if guild is None:
        log.error("Bot ist auf keinem Guild oder GUILD_ID falsch.")
    else:
...
async def dryrun(interaction: discord.Interaction, source: Optional[str] = None):
    ...
    src = source or AGENT_SOURCE_URL or "blueprint.yaml"
    bp = load_blueprint(src)
...
async def lockdown(interaction: discord.Interaction):
    ...
    for ch in interaction.guild.text_channels:
        if ch.category and ch.category.name.startswith("🔒 "):
            continue
